...
func load_profile() -> void:
	...
	Log.info("[UserService][load_profile] Profile loaded successfully for user: %s" % _current_profile["username"])
	profile_loaded.emit()
...
func start_session() -> void:
	...
	Log.info("[UserService][start_session] Session started, total sessions: %d" % _current_profile["total_sessions"])
...
func set_username(p_new_username: String) -> void:
	...
	save_profile()
	Log.info("[UserService][set_username] Username changed from %s to %s" % [old_username, p_new_username])


## Add achievement to user profile
func unlock_achievement(p_achievement_id: String) -> bool:
	Log.info("[UserService][unlock_achievement] Attempting to unlock achievement: %s" % p_achievement_id)

	if p_achievement_id in _current_profile["achievements"]:
		Log.info("[UserService][unlock_achievement] Achievement already unlocked: %s" % p_achievement_id)
...
func has_achievement(p_achievement_id: String) -> bool:
	...
	Log.info("[UserService][has_achievement] Checking achievement %s: %s" % [p_achievement_id, has_it])
	return has_it


## Get all unlocked achievements
func get_achievements() -> Array:
	Log.info("[UserService][get_achievements] Getting all achievements (%d total)" % _current_profile["achievements"].size())
...
func import_profile(p_import_path: String) -> bool:
	...
	if success:
		Log.info("[UserService][import_profile] Profile imported successfully for user: %s" % _current_profile["username"])
	else:
...
func _update_profile_from_session(p_session_results: Dictionary) -> void:
	...
	var experience_gained = _calculate_experience_gain(p_session_results)
	var old_level = _current_profile["level"]

	_current_profile["total_time_typed"] += session_time
	_current_profile["experience"] += experience_gained

	Log.info("[UserService][_update_profile_from_session] Session time: %.1fs, Experience gained: %d" % [session_time, experience_gained])
	...
	if new_level > _current_profile["level"]:
		...
		Log.info("[UserService][_update_profile_from_session] Level up! %d -> %d" % [old_level, new_level])
...
func _calculate_experience_gain(p_session_results: Dictionary) -> int:
	...
	Log.info("[UserService][_calculate_experience_gain] Base: %d, WPM bonus: %d, Accuracy bonus: %d, Total: %d" % [base_exp, wpm_bonus, accuracy_bonus, total_exp])
	return total_exp


func _calculate_level_from_experience(p_experience: int) -> int:
	# Simple level calculation: every 1000 exp = 1 level
	var level = max(1, int(p_experience / 1000.0) + 1)
	Log.info("[UserService][_calculate_level_from_experience] Experience: %d -> Level: %d" % [p_experience, level])
	return level
...
func _check_for_achievements(p_session_results: Dictionary) -> void:
	...
	Log.info("[UserService][_check_for_achievements] Achievement check complete, %d new achievements unlocked" % achievements_unlocked)
...
class SessionStats:
	...
	func end_session(p_results: Dictionary) -> void:
		...
		is_active = false
		Log.info("[UserService.SessionStats][end_session] Final stats - WPM: %.1f, Accuracy: %.1f%%, Characters: %d, Mistakes: %d" % [current_wpm, current_accuracy, characters_typed, mistakes])
...
class ProfileStats:
	...
	func load_from_profile(p_profile: Dictionary) -> void:
		...
		Log.info("[UserService.ProfileStats][load_from_profile] Loaded stats - Sessions: %d, Best WPM: %.1f, Best Accuracy: %.1f%%" % [sessions_completed, best_wpm, best_accuracy])
	...
	func update_with_session(p_session_results: Dictionary) -> void:
		Log.info("[UserService.ProfileStats][update_with_session] Updating profile stats with session data")
		...
		Log.info("[UserService.ProfileStats][update_with_session] Stats updated - New best WPM: %s (%.1f), New best accuracy: %s (%.1f%%)" % [new_best_wpm, best_wpm, new_best_accuracy, best_accuracy])
