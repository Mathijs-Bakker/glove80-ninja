...
# UI References
@onready
var cursor_style_option: OptionButton = $VBoxContainer/SettingsContainer/CursorStyleContainer/CursorStyleOption
@onready var theme_option: OptionButton = $VBoxContainer/SettingsContainer/ThemeContainer/ThemeOption
@onready
var font_size_spinbox: SpinBox = $VBoxContainer/SettingsContainer/FontSizeContainer/FontSizeSpinBox
@onready
var sound_volume_slider: HSlider = $VBoxContainer/SettingsContainer/SoundVolumeContainer/SoundVolumeSlider
@onready
var typing_sounds_checkbox: CheckBox = $VBoxContainer/SettingsContainer/TypingSoundsContainer/TypingSoundsCheckBox
@onready
var language_option: OptionButton = $VBoxContainer/SettingsContainer/LanguageContainer/LanguageOption
...
func _load_current_settings() -> void:
	...
	Log.info(
		(
			"[SettingsController][_load_current_settings] Loaded %d settings from config service"
			% original_settings.size()
		)
	)
...
func _apply_settings_to_ui() -> void:
	...
	if cursor_style_option:
		...
		Log.info(
			"[SettingsController][_apply_settings_to_ui] Set cursor style to: %s" % cursor_style
		)
	...
	if sound_volume_slider:
		...
		Log.info(
			"[SettingsController][_apply_settings_to_ui] Set sound volume to: %d" % sound_volume
		)
	...
	if typing_sounds_checkbox:
		...
		Log.info(
			"[SettingsController][_apply_settings_to_ui] Set typing sounds to: %s" % typing_sounds
		)
...
func save_settings() -> void:
	...
	if save_success:
		Log.info(
			(
				"[SettingsController][save_settings] %d settings saved successfully to disk"
				% settings_saved
			)
		)
	else:
...
func close_settings() -> void:
	...
	if has_unsaved_changes:
		(
			Log
			. info(
				"[SettingsController][close_settings] Unsaved changes detected, showing confirmation dialog"
			)
		)
		var confirm_dialog = ConfirmationDialog.new()
...
func _update_button_states() -> void:
	(
		...
		. info(
			(
				"[SettingsController][_update_button_states] Updating button states - unsaved changes: %s"
				% has_unsaved_changes
			)
		)
	)
...
func _get_cursor_style_index(p_style: String) -> int:
	Log.info(
		"[SettingsController][_get_cursor_style_index] Getting index for cursor style: %s" % p_style
	)
	match p_style:
		"block":
			return 0
		...
		_:
			(
				...
				. warn(
					(
						"[SettingsController][_get_cursor_style_index] Unknown cursor style: %s, defaulting to block"
						% p_style
					)
				)
			)
...
func _get_theme_index(p_theme_name: String) -> int:
	...
	match p_theme_name:
		...
		_:
			Log.warn(
				(
					"[SettingsController][_get_theme_index] Unknown theme: %s, defaulting to dark"
					% p_theme_name
				)
			)
			return 0
...
func _get_language_index(p_language: String) -> int:
	Log.info(
		"[SettingsController][_get_language_index] Getting index for language: %s" % p_language
	)
	match p_language:
		"en":
			return 0
		...
		_:
			(
				...
				. warn(
					(
						"[SettingsController][_get_language_index] Unknown language: %s, defaulting to English"
						% p_language
					)
				)
			)
...
func _on_cursor_style_changed(p_index: int) -> void:
	Log.info(
		"[SettingsController][_on_cursor_style_changed] Cursor style changed to index: %d" % p_index
	)
...
func _on_sound_volume_changed(p_value: float) -> void:
	Log.info(
		"[SettingsController][_on_sound_volume_changed] Sound volume changed to: %.1f" % p_value
	)
	has_unsaved_changes = true
	_update_button_states()
...
func _on_typing_sounds_toggled(p_pressed: bool) -> void:
	Log.info(
		"[SettingsController][_on_typing_sounds_toggled] Typing sounds toggled to: %s" % p_pressed
	)
	has_unsaved_changes = true
	_update_button_states()
...
func _on_setting_changed(p_setting_name: String, p_new_value) -> void:
	Log.info(
		(
			"[SettingsController][_on_setting_changed] External setting changed: %s = %s"
			% [p_setting_name, p_new_value]
		)
	)
			...
			KEY_ENTER:
				if has_unsaved_changes:
					(
						Log
						. info(
							"[SettingsController][_input] Enter key pressed with unsaved changes, saving settings"
						)
					)
					save_settings()
			KEY_S:
				if key_event.ctrl_pressed and has_unsaved_changes:
					(
						Log
						. info(
							"[SettingsController][_input] Ctrl+S pressed with unsaved changes, saving settings"
						)
					)
					save_settings()
