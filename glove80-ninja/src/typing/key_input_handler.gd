...
var _typing_controller: TypingController


func _init(p_controller: TypingController) -> void:
	_typing_controller = p_controller
...
func _handle_character_input(p_key_event: InputEventKey) -> bool:
	...
	var expected_char = _typing_controller.get_target_text()[CursorPosition.idx().current]

	return true
	
	# Process the character
	if _is_character_correct(character, expected_char):
...
func _is_input_complete() -> bool:
	var text_length = _typing_controller.get_target_text().length()
	return CursorPosition.idx().current >= text_length
